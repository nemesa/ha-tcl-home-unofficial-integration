...
def get_SplitAC_Type2_capabilities() -> list[list[int]]:
    capabilities1 = [
        2,
        3,
        7,
...
async def get_stored_spit_ac_type2_data(
    ...
    stored_data, need_save = safe_set_value(
        stored_data, "user_config.behavior.memorize_temp_by_mode", True
    )
    stored_data, need_save = safe_set_value(
        stored_data, "user_config.behavior.memorize_fan_speed_by_mode", True
    )
    stored_data, need_save = safe_set_value(
    ...
    stored_data, need_save = safe_set_value(
        stored_data, "fan_speed.Cool.value", WindSeed7GearEnum.AUTO
    )
    stored_data, need_save = safe_set_value(
        stored_data, "fan_speed.Heat.value", WindSeed7GearEnum.AUTO
    )
    stored_data, need_save = safe_set_value(
        stored_data, "fan_speed.Dehumidification.value", WindSeed7GearEnum.AUTO
    )
    stored_data, need_save = safe_set_value(
        stored_data, "fan_speed.Fan.value", WindSeed7GearEnum.AUTO
    )
    stored_data, need_save = safe_set_value(
        stored_data, "fan_speed.Auto.value", WindSeed7GearEnum.AUTO
    )
